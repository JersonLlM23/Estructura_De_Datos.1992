...
void ListaLibros::filtrarPorAnio() {
    ...
    // Lambda para validar si un string es un número
    auto esNumero = [](const string& str) -> bool {
        return std::regex_match(str, std::regex("\\d+"));
    };

    // Solicitar y validar el rango de años
    while (true) {
        // Solicitar año inicial
        while (true) {
            cout << "Ingrese el año inicial: ";
            cin >> inputInicio;
            if (!esNumero(inputInicio)) {
        ...
        while (true) {
            ...
            if (!esNumero(inputFinal)) {
                ...
                if (anioFinal >= anioInicio) {
                    ...
                    cout << "Error: El año final debe ser mayor o igual al año inicial.\n";
                    cout << "Por favor, vuelva a ingresar el año inicial.\n";
                    anioInicio = 0; // Reiniciar el año inicial
                    break;
                }
            }
        }

        // Salir si ambos años son válidos
        if (anioInicio > 0 && anioFinal >= anioInicio) {
            break;
        }
    }
